...
**strength 只描述这一条关系本身,不要把别的事情算进去。**
每种类型都有一个通常的强度(室友通常 +2、朋友 +2、点头之交 0、竞争 -2……),
除非材料明确说明这段关系比通常更近或更远,否则就用通常值。

特别注意:一对人之间同时有正面和负面关系时(比如"是室友但闹了别扭"),
要**分别输出两条**,各自保持自己该有的强度 —— 室友还是 +2,摩擦是 -1。
**不要把负面情绪折算进正面那条的强度里**(比如因为闹了别扭就把室友写成 0)。
那样会把"关系复杂"抹平成"关系不好",而前者恰恰是更值得记录的信息。
